...
**Мой reasoning:** _По теории kit отличается от папки заметок тем, что отвечает на вопрос «какой сценарий — что запустить — как принять — когда остановиться». Именно карта применения связывает элементы в рабочий набор, а не количество или длина файлов._
...
- Связь сценария с reusable-элементом
- Пример удачного запуска
- Review skill для разных проектов
- Команда тестов текущего репозитория
...
**Мой reasoning:** _Личный уровень — переносимые формы вроде review skill; проектный — локальные команды вроде запуска тестов; карта применения связывает сценарий с тем, что запускать; good run — заметка об удачном реальном запуске._
...
**Варианты:**
- [✗ выбран, неверно] Ни в одном из перечисленных инструментов
-  Codex
-  Claude Code
-  Cursor
...
**Мой reasoning:** _В теории явно указано: Claude Code хранит личные skills в ~/.claude/skills/, Codex — в ~/.agents/skills/, Cursor поддерживает User Rules. У всех трёх есть встроенный личный уровень._
...
**Варианты:**
- [✓ правильный] Удаление устаревших элементов
- [✓ правильный] Проверка на дублирование
- [✓ правильный] Правки после реального запуска
-  Добавление всех найденных промптов

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Теория прямо называет три практики поддержки kit: правки сразу после применения, удаление устаревших элементов и ревизия на дублирование. Добавление всех найденных промптов противоречит принципу «kit должен становиться короче и точнее»._
...
- [✓ правильный] Чтобы убрать устаревший шум
-  Чтобы скрыть историю изменений

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано: устаревшие шаблоны и rules без применения создают шум, а старый template опасен тем, что выглядит рабочим — поэтому его удаляют._
...
**Варианты:**
- [✓ правильный] Стандартизированный handoff
- [✓ правильный] Повторяемый review diff
- [✓ правильный] Debugging brief без правки кода
-  Спорное изменение биллинга

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Теория прямо называет эти три сценария как места, где kit повышает автономность: review diff с checklist, debugging brief без изменения кода и стандартизированный handoff. Спорное изменение биллинга — рискованный сценарий, где kit должен возвращать решение человеку._
...
- [✓ правильный] При спорных решениях
-  При чтении документации

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Теория говорит, что в рискованных сценариях (публичный API, миграции, безопасность, необратимые операции) kit должен раньше возвращать решение человеку. Спорные решения — это как раз ситуации, где нужен ранний stop-point и подтверждение человека._
