...
**Варианты:**
- [✓ правильный] Процесс остался в чате
...
**Мой reasoning:** _В теории прямо сказано: если маршрут, критерии и стоп-сигналы существуют только внутри одного диалога, в следующей сессии агент их не помнит и они не становятся частью системы разработки._
...
**Варианты:**
- [✓ правильный] У него есть проверяемый результат
- [✓ правильный] Он повторяется в похожих задачах
-  Он требует нового архитектурного решения
- [✓ правильный] Его каждый раз нужно объяснять заново

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Теория прямо называет три признака: повторяемость, стабильный маршрут (раздражение от повторных объяснений одних и тех же ограничений) и проверяемый результат. Сценарии с новым архитектурным решением, наоборот, отнесены к слабым кандидатам — их лучше оставить в ручном управлении._
...
- Описывает переносимую процедуру с границами
- Помогает принять результат по критериям
- Стабилизирует постановку похожей задачи
- Запускает стандартный маршрут коротким входом
...
**Мой reasoning:** _По теории: prompt template решает проблему переформулирования постановки, checklist — слабой приёмки, command — частого запуска одной операцией, skill — переносимая процедура с условиями применения, материалами и стоп-сигналами._
...
**Варианты:**
- [✓ правильный] Разовый запрос с контролем
...
**Мой reasoning:** _Теория прямо говорит: разовая исследовательская или спорная задача без стабильного маршрута требует плотного ручного управления, а не упаковки. Skill, команда или общее правило преждевременны — нет повторяемости, стабильного маршрута и проверяемого результата._
...
**Варианты:**
- [✓ правильный] Критерии приёмки результата
-  Всю историю чата без отбора
- [✓ правильный] Входы и порядок действий
- [✓ правильный] Сценарий и условия применения

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Теория явно перечисляет рабочий минимум: сценарий, входы, порядок действий, результат и приёмку. Полный чат сохранять не нужно — фиксируется только рабочая форма._
...
- [✓ правильный] Checklist приёмки diff

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Проблема в нестабильной приёмке — агент пропускает тесты и смешивает блокирующие замечания с улучшениями. По теории это прямой триггер для checklist: самой лёгкой формы, которая закрепляет критерии приёмки без преждевременного раздувания до skill._
...
- [✓ правильный] Проверяем ли его результат?

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо названы три вопроса проверки зрелости сценария: повторяется ли, стабилен ли маршрут, проверяем ли результат. Из предложенных вариантов только этот совпадает с критериями приёмки._
...
**Варианты:**
- [✓ правильный] Review AI-кода перед merge
- [✓ правильный] Подготовка feature brief
- [✓ правильный] Release prep с проверками
-  Разовый выбор архитектуры продукта

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Теория прямо называет review AI-кода, release prep и feature brief среди типичных кандидатов на упаковку — у них стабильные входы и проверяемый результат. Разовый выбор архитектуры — слабый кандидат, формулировка слишком широкая._
...
- [✓ правильный] Rules фиксируют постоянные ограничения
-  Skill обязан содержать все правила проекта
-  Rules нужны только для дизайна интерфейса

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано: проектные правила отвечают за постоянные ограничения, а skill — за повторяемую работу в конкретном классе задач. Поэтому skill не должен дублировать AGENTS.md/CLAUDE.md/rules._
...
**Варианты:**
- [✓ правильный] Когда применять сценарий
- [✓ правильный] Подходящая форма фиксации
- [✓ правильный] Ожидаемый результат
-  Рейтинг всех доступных моделей

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо указано, что для каждого сценария в карте нужно указать когда применять, входы, ожидаемый результат, критерии приёмки и подходящую форму фиксации. Рейтинг моделей не упоминается._
