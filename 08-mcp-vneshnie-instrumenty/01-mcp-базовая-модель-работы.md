...
**Варианты:**
- [✓ правильный] Model Context Protocol
...
**Мой reasoning:** _В теории прямо указано: 'Model Context Protocol (MCP) — открытый стандарт', опубликованный Anthropic в ноябре 2024 года._
...
- [✓ правильный] Приложение, встраивающее агента и поддержку MCP

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано: Host — это приложение, которое запускает агента и встраивает поддержку MCP (Claude Code, Cursor и т.д.). Остальные варианты описывают client, server или сам протокол._
...
**Мой reasoning:** _По теории, stdio — транспорт по умолчанию для локальных MCP-серверов: host запускает сервер как дочерний процесс и общается через stdin/stdout. Streamable HTTP — для удалённых, а SSE-only считается устаревшим._
...
- [✓ правильный] Он устарел и не рекомендуется для новых интеграций
-  Он заменяет stdio для локальных серверов
-  Это актуальный стандарт для удалённых серверов

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано, что старый транспорт HTTP+SSE из версии протокола 2024-11-05 устарел и не рекомендуется для новых интеграций; актуальный транспорт для удалённых серверов — Streamable HTTP._
...
- [✓ правильный] Данные по URI без побочных эффектов
-  Функция, создающая GitHub PR
-  Действие с записью в базу данных

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _По теории resource в MCP — это данные, адресуемые URI (file:///..., postgres://...), которые агент читает без изменения внешнего состояния, аналог GET-запроса._
...
- Шаблон «проанализируй медленный SQL-запрос»
- create_issue в GitHub-сервере
- Компонент внутри host, управляющий соединением
...
**Мой reasoning:** _Tool — действие с побочным эффектом (create_issue); Resource — данные по URI (file://); Prompt — встроенный шаблон; Client — компонент host, управляющий соединением с сервером._
...
**Варианты:**
- [✓ правильный] Отправить сообщение в Slack после агентного прохода
...
**Мой reasoning:** _Это внешнее действие с побочным эффектом в стороннем сервисе — ровно тот случай, для которого нужен MCP-сервер. Остальные сценарии покрываются template, hook и skill/command соответственно._
...
- [✓ правильный] Стандартизация review без доступа к внешним данным

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Skill задаёт процедуру агенту без обращения к внешнему состоянию. Остальные варианты требуют доступа к внешним данным или действий с побочными эффектами — это случаи для MCP._
...
**Варианты:**
- [✓ правильный] MCP — открытый стандарт, не привязанный к Anthropic
-  MCP заменяет skills и commands
-  SSE-транспорт рекомендован для новых удалённых серверов
- [✓ правильный] Подключение нового сервера расширяет область действий агента

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано, что каждый добавленный сервер расширяет область действий агента, и что спецификация MCP открыта и не привязана к конкретному провайдеру. SSE устарел, а MCP не заменяет skills/commands — это другой слой._
