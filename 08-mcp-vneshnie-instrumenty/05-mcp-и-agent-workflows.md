...
- [✓ правильный] Встраивается как один из шагов skill-процедуры
...
**Мой reasoning:** _В теории прямо сказано: 'Процедура может включать вызов инструмента как один из шагов — и это делает skill мощнее'. MCP не заменяет skill, а дополняет его как шаг внутри процедуры._
...
**Варианты:**
- [✓ правильный] Каждый сервер задокументирован и привязан к workflows
...
**Мой reasoning:** _В теории прямо сказано: любой сервер должен иметь запись в docs/mcp-servers.md и ссылку из docs/reusable-workflows.md — то есть документация плюс привязка к конкретным workflows._
...
**Варианты:**
- [✓ правильный] Что предоставляет сервер и какие workflows его используют
...
**Мой reasoning:** _В теории прямо сказано: структура раздела — одна строка о том, что предоставляет сервер, и список 'Участвует в:' с workflows. Полный контракт лежит отдельно в docs/mcp-contracts.md._
...
- [✓ правильный] При добавлении нового MCP-сервера
- [✓ правильный] При изменении skill или command, использующих сервер
- [✓ правильный] При изменении scope токена существующего сервера

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Теория прямо перечисляет эти три события в разделе 'Когда обновлять документацию'. Каждый агентный сеанс — не триггер обновления, документация обновляется по изменениям конфигурации и workflows._
...
- [✓ правильный] Раз в месяц или при смене задач

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В разделе «Ревизия kit: раз в месяц или при смене задач» прямо указана эта периодичность с описанием чеклиста на 15 минут._
...
- [✓ правильный] Подключены серверы «с прошлого проекта» без документации

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо перечислены признаки выхода MCP-слоя из-под контроля: серверы, оставшиеся с прошлого проекта, и отсутствие документации. Остальные варианты описывают здоровое состояние kit._
...
- Конфигурация подключений к серверам
- Описание workflows и их MCP-зависимостей
- Контракт каждого MCP-сервера
- Карта инструментов с режимами и стоп-точками
...
**Мой reasoning:** _mcp-servers.md хранит контракты серверов, reusable-workflows.md связывает workflows с MCP-зависимостями, autonomy-map.md — таблица инструментов с режимами/стоп-точками, .mcp.json — сама конфигурация подключений._
...
- [✓ правильный] Убрать из конфига и документации, отозвать токен

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории явно прописан чеклист удаления: убрать секцию из .mcp.json, удалить запись в docs/mcp-servers.md, убрать ссылки из reusable-workflows.md и autonomy-map.md, отозвать токен. Это единственный вариант, охватывающий все три части — конфиг, документацию и токен._
...
- [✓ правильный] Добавить её в личный dotfiles-репозиторий без секретов

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано: личная конфигурация не отслеживается автоматически, и если есть личный dotfiles-репозиторий, добавьте туда и конфигурацию MCP без секретов — тогда при переходе на новую машину не придётся вспоминать настройки._
