...
- [✓ правильный] Список tools всех серверов загружается в контекст
...
**Мой reasoning:** _В теории прямо сказано: в начале каждой сессии host опрашивает все подключённые серверы и список tools/resources/prompts загружается в контекстное окно агента — это и есть контекстное расширение._
...
**Варианты:**
- [✓ правильный] Внедрение вредоносных инструкций в ответ MCP-сервера
...
**Мой reasoning:** _В теории прямо сказано: вредоносный сервер может вернуть в ответе tool не данные, а инструкции для агента, маскированные под данные. Это и есть промпт-инъекция через MCP._
...
**Мой reasoning:** _Из таблицы урока: Git read-only локально — низкий; Slack write во внешнюю публичную систему — высокий; PostgreSQL read-only — низкий–средний; GitHub с write-операциями (PR, issues) во внешнем состоянии — средний._
...
-  Dry-run первым
- [✓ правильный] Разделение сессий по уровню доступа

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо описан Принцип 4 'Разделение сессий по уровню доступа' с примерами аналитической, рабочей и коммуникационной сессий — именно той формулировкой, что в вопросе._
...
**Варианты:**
- [✓ правильный] Потому что вредоносный PR с ним затронет всю команду
...
**Мой reasoning:** _В теории прямо сказано: если злоумышленник добавит в PR изменение .mcp.json с вредоносным сервером, все участники команды, принявшие PR, подключат его автоматически. Поэтому конфигурация MCP должна проходить code review как security-критичный файл._
...
- [✓ правильный] Создать токен с минимальным scope для нужного репо
-  Отключить GitHub-сервер

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Принцип минимального scope из теории: токен должен иметь наименьшие необходимые права. Широкий токен — это не «подключил GitHub-сервер», а «отдал агенту доступ ко всем репозиториям»._
...
- [✓ правильный] Агент без запроса объединяет Filesystem и Slack

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо описан этот сценарий: при одновременно подключённых Filesystem и Slack агент технически может прочитать локальный файл и отправить его в Slack — никто не просил, но ничто не запретило. Это и называют нежелательным соединением возможностей._
...
- [✓ правильный] Только явный вызов пользователем
-  С подтверждением при первом запуске
-  Автономно

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо в таблице связи MCP-инструментов с картой автономности send_message (Slack) отнесён к высокому риску с режимом 'Только явный вызов пользователем'. Это write-операция во внешнюю публичную систему с необратимыми последствиями._
...
**Варианты:**
- [✓ правильный] Создавать токены с минимально необходимым scope
- [✓ правильный] Подключать только серверы, нужные для текущей задачи
- [✓ правильный] Начинать с read-only, расширять до write только когда нужно
-  Держать все серверы подключёнными постоянно для удобства

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Это три из шести принципов ограничения, прямо сформулированные в теории. Постоянное подключение всех серверов противоречит принципу 1 о минимальном наборе серверов на сессию._
