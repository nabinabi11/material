...
**Варианты:**
- [✓ правильный] С просмотра ветки, статуса и diff
...
**Мой reasoning:** _В теории прямо сказано: начинаем с короткого цикла — ветка, статус, карта файлов, детальный diff. Сначала факты, потом решения о staging и коммите._
...
- [✓ правильный] Он связан с одной причиной изменения
-  Его сообщение перечисляет максимум деталей diff

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано: «Хороший коммит связан с одной причиной. Он не обязан быть микроскопическим, но у него должна быть ясная граница». Остальные варианты противоречат принципу узкой границы коммита._
...
- [✓ правильный] Чтобы собрать нужный срез изменений
-  Чтобы агент автоматически сделал PR

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Из теории: staging — промежуточный слой между сессией и историей, который позволяет взять из рабочего дерева не всё подряд, а только тот срез, который действительно хотим сохранить. Это превращает неидеальный агентный проход в понятный инженерный результат._
...
**Варианты:**
- [✓ правильный] Что цель и проверка названы ясно
...
**Мой reasoning:** _По теории, PR-текст держится на четырёх вещах: цель, что вошло, как проверяли, что вне scope — без общих фраз. Остальные варианты прямо противоречат принципу узкого и сфокусированного описания._
...
**Варианты:**
- [✗ выбран, неверно] Держать вместе код и тесты по одной задаче
- [✗ выбран, неверно] Объяснить commit одной фразой
- [✗ выбран, неверно] Отложить попутные изменения отдельно
- [✗ выбран, неверно] Использовать staging по смысловым кускам

**Статус Stepik:** `wrong` (score 0.0)

**Мой reasoning:** _Теория описывает все четыре приёма как способы удержать границу коммита: одна причина (код+тесты по задаче вместе), вынесение попутной полировки отдельно, staging по смысловым кускам через git add -p и тест «объясни одной фразой»._
...
**Варианты:**
-  промпт агенту
- [✓ правильный] Что вошло в изменения
- [✓ правильный] Как проверяли результат
- [✓ правильный] Цель изменения

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо названы четыре пункта PR-описания: цель, что вошло, как проверяли и что осталось вне scope. Промпт агенту в этот список не входит._
...
- Дать детали по строкам
- Быстро показать карту изменений
- Показать текущее состояние файлов
- Отобрать только нужные hunks
...
**Мой reasoning:** _По теории: --stat = карта файлов без деталей, git diff = подробный diff по строкам, add -p = интерактивный отбор hunks, status --short = краткое состояние рабочего дерева._
...
**Колонка А (вопросы):**
- В diff попала косметика рядом с фиксом
- Hunk слишком крупный
- Агент предложил commit message
- Рабочее дерево уже чистое

**Колонка Б (варианты, перемешаны):**
- Оценить ширину недавних коммитов
- Попробовать `s` и разбить его
- Проверить смысл и границу
- Не тащить всё в один commit
...
**Мой reasoning:** _Косметика рядом с фиксом — повод разделить на два коммита; крупный hunk дробится через s в git add -p; предложенный агентом message нужно проверить, так как агент иногда описывает весь diff без разбора; при чистом дереве практика сводится к оценке ширины уже сделанных коммитов (сценарий Б)._
...
**Колонка Б (варианты, перемешаны):**
- Отделить нужный срез от лишних правок
- Коротко объяснить что и зачем изменили
- Показать цель, проверку и границы задачи
- Сохранить одну инженерную причину в истории
...
**Мой reasoning:** _Commit message объясняет что и зачем; staging выделяет срез из рабочего дерева; PR-описание раскрывает цель/проверку/вне scope; узкий коммит хранит одну причину в истории._
