...
- [✓ правильный] Внешний контракт, ошибки сети и конфигурация

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Теория прямо говорит: с появлением внешнего сервиса добавляются внешний контракт, чужие ошибки, таймауты, лимиты и секреты — то есть контракт, сетевые отказы и конфигурация. Остальные варианты либо не упомянуты, либо противоречат идее узкого первого прохода._
...
**Варианты:**
- [✓ правильный] Чтобы зафиксировать контракт, ошибки и конфигурацию
...
**Мой reasoning:** _Согласно теории, integration brief нужен чтобы заранее зафиксировать внешний контракт, точки отказа, конфигурацию и границы прохода — иначе агент примет эти решения молча._
...
- [✓ правильный] Минимальный адаптер под один сценарий и один контракт
-  Реализовать все возможные ретраи, fallback и circuit breaker

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Теория прямо указывает: первый проход — узкий адаптер под один endpoint и одну точку вызова, без общей платформы, ретраев и fallback. Это каркас, который дешевле пересобрать и проще принять по diff._
...
**Варианты:**
- [✓ правильный] Базовые негативные кейсы и точки отказа
- [✓ правильный] Формат входа и выхода внешнего контракта
-  Полный мониторинг будущего продакшена
- [✓ правильный] Источник конфигурации и правила работы с секретами

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Integration brief фиксирует контракт (вход/выход), ошибки и точки отказа, а также конфигурацию и работу с секретами. Полный мониторинг продакшена — это уже следующие этапы, явно откладываемые из первого прохода._
...
- [✓ правильный] Очередь и расширенная обвязка
- [✓ правильный] Ретраи и backoff
- [✓ правильный] Fallback-провайдер

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _По теории на следующий этап откладываются ретраи и backoff, fallback-провайдер, очередь и расширенная обвязка. Минимальный адаптер под один endpoint — это и есть содержание первого прохода, его откладывать нельзя._
...
**Варианты:**
- [✓ правильный] Отсутствие секретов в коде и логах
- [✓ правильный] Поведение без конфигурации или при ошибке внешнего сервиса
- [✓ правильный] Happy path по целевому сценарию
-  Только длину сгенерированного diff

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Теория прямо называет приёмкой проверку успешного пути, минимум одного негативного кейса (нет конфигурации, таймаут, 4xx/5xx, пустой ответ) и контроль логов/diff на утечку секретов. Длина diff сама по себе не критерий — важна его узость и отсутствие лишней платформы._
...
**Варианты:**
- [✓ правильный] Агент молча добавил платформенные абстракции без запроса
- [✓ правильный] Для одного webhook появился общий integration layer «на будущее»
- [✓ правильный] Diff затрагивает много файлов вне целевого сценария
-  В коде есть один локальный адаптер под один контракт

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Признаки переусложнения — широкий diff за пределами сценария, незапрошенные платформенные абстракции и общий integration layer ради одного webhook. Локальный адаптер под один контракт — наоборот, признак правильного узкого прохода._
...
- Какие негативные случаи учитываем сразу
- Что отправляем и что ожидаем получить
- Что сознательно не делаем в первом этапе
- Где хранятся URL, токены и настройки
...
**Мой reasoning:** _Прямое сопоставление по теории: контракт = вход/ожидаемый выход, конфигурация = хранение секретов и URL, точки отказа = негативные случаи (таймауты, ошибки), границы прохода = что откладываем на следующий этап._
...
**Колонка А (вопросы):**
- Токен зашит в пример кода
- Есть только успешный локальный вызов
- Адаптер подключён в один сценарий
- В diff появилась очередь и fallback

**Колонка Б (варианты, перемешаны):**
- Приёмка опирается лишь на happy path
- Первый проход остаётся локальным
- Нарушена граница безопасности
- Интеграция расползлась сверх brief
...
**Мой reasoning:** _Сопоставление по теории: хардкод секрета = нарушение безопасности, только happy path = неполная приёмка, узкий адаптер = локальный первый проход, очередь и fallback = выход за рамки brief._
...
**Колонка А (вопросы):**
- Описать endpoint, payload и ошибки
- Попросить минимальный адаптер
- Проверить отсутствие токена и таймаут
- Отдельно записать ретраи и мониторинг

**Колонка Б (варианты, перемешаны):**
- Сборка integration brief
- Негативные кейсы приёмки
- Первый инженерный проход
- Следующий этап после первого прохода
...
**Мой reasoning:** _Brief фиксирует контракт и ошибки; первый проход — узкий адаптер; приёмка включает негативные кейсы (нет токена, таймаут); ретраи и мониторинг откладываются на следующий этап._
