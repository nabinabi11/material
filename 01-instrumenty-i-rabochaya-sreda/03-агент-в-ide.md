...
- [✓ правильный] Форк меняет ядро, расширение живёт в API редактора
...
**Мой reasoning:** _В теории прямо сказано, что Cursor и Windsurf — форки VS Code с переработанным ядром под агентную разработку, тогда как расширения вроде Copilot/Cline/Roo Code добавляются поверх привычной среды через API редактора. Остальные варианты либо фактически неверны (расширения умеют выполнять команды), либо не являются принципиальным архитектурным отличием._
...
- [✓ правильный] Plan строит план без правок до подтверждения

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано: режим Plan строит интерактивный план с to-do списком, но не вносит изменения в код — это и есть ключевое отличие от Agent, который сразу правит файлы._
...
- [✓ правильный] Cascade видит открытый код и диагностику без @

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано: real-time awareness — Cascade учитывает текущий контекст редактора (открытые файлы, активный код, диагностические сообщения), что снижает необходимость явно передавать контекст._
...
- [✓ правильный] Векторный поиск находит код по смыслу запроса
-  Qdrant кэширует поиск и отвечает быстрее grep
-  Семантический поиск учитывает историю файла при ранжировании

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано, что Roo Code конвертирует код в эмбеддинги и ищет по смыслу запроса, а не по точному совпадению текста — поэтому находит логику аутентификации, даже если слова «аутентификация» в коде нет._
...
**Варианты:**
- [✓ правильный] Цена задачи предсказуема независимо от её длины
...
**Мой reasoning:** _В теории прямо сказано: стоимость привязана к количеству сообщений (кредитов), а не к числу выполненных действий/токенов, что делает расходы более предсказуемыми._
...
- ценообразование по кредитам за сообщение, а не по токенам
- Полноценная IDE с агентными функциями на уровне ядра
- Возможность индексирования кодовой базы
- Официальная поддержка JetBrains и Neovim
...
**Мой reasoning:** _Copilot — единственный с плагинами для JetBrains и Neovim. Windsurf тарифицирует по кредитам за промпт. Cursor — AI-native IDE на форке VS Code. Roo Code славится семантическим индексированием через Qdrant._
...
**Варианты:**
- [✓ правильный] Снимает все подтверждения действий агента
...
**Мой reasoning:** _В теории прямо сказано: Run Everything — это режим автоодобрения всех действий агента, когда Cursor не запрашивает подтверждение перед значимыми действиями._
