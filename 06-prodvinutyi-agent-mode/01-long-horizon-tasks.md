...
- [✓ правильный] Длинная цепочка этапов

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории явно сказано, что проблема long-horizon задачи не в размере, а в длине траектории — цепочке зависимых решений и этапов. Остальные варианты (объём кода, цена, срочность) не определяют long-horizon природу задачи._
...
**Варианты:**
- [✓ правильный] Размывает момент ручной проверки
-  Упрощает возврат после паузы
- [✓ правильный] Делает побочные повороты незаметными
- [✓ правильный] Скрывает текущий этап работы

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _По теории монолитный запрос теряет инженерную прозрачность: непонятно на каком этапе агент, побочные повороты выглядят как естественное продолжение, теряется точка ручной проверки. Возврат после паузы он не упрощает, а наоборот усложняет._
...
- Внешний проверяемый артефакт
- Признак запрета на автопереход
- Наблюдаемый итог маршрута
- Кусок работы с одной функцией
...
**Мой reasoning:** _Цель = наблюдаемый итог маршрута, этап = однородный кусок работы с одной функцией, контрольная точка = внешний артефакт (а не ощущение), сигнал остановки = условие, запрещающее автоматический переход дальше._
...
- [✓ правильный] Зафиксированный diff на узком участке
- [✓ правильный] Список подтверждённых допущений
- [✓ правильный] Документ с решением

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Контрольная точка должна быть внешним артефактом: документ с решением, фиксирующий коммит/diff, список подтверждённых допущений. Ощущение прогресса прямо названо в теории бесполезным сигналом._
...
- [✓ правильный] Сигналы остановки для маршрута
-  Все возможные побочные улучшения продукта

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано: сигналы остановки лучше определить заранее, до старта прохода, чтобы агент не двигался дальше автоматически при появлении рисков. Остальные варианты либо вредны (расползание границ), либо невозможны до начала работы._
...
**Варианты:**
- [✓ правильный] Что уже подтверждено
- [✓ правильный] Текущий этап
- [✓ правильный] Что делать следующим шагом
-  Полная история всех обсуждений

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории заметка передачи контекста отвечает на четыре вопроса: где мы находимся (текущий этап), что подтверждено, что делаем следующим шагом и какие риски. Полная история обсуждений — это лог чата, который явно назван плохой опорой для возобновления._
...
- [✓ правильный] Держать заметку в AGENTS.md или аналоге
-  Хранить статус только в памяти чата
-  Писать статус лишь в названии рабочей git-ветки

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В уроке прямо сказано: в Codex заметку передачи контекста удобно держать в AGENTS.md — агент читает его при каждом новом запуске._
...
- Расползание границ
- Потеря цели
- Ранний архитектурный перегиб
- Неподтверждённые допущения

**Колонка Б (варианты, перемешаны):**
- Диффы идут, но этап маршрута неясен
- Побочные улучшения маскируются под прогресс
- Решения строятся на «должно работать»
- Усложнение до проверки базового пути
...
**Мой reasoning:** _Расползание границ описано через побочные улучшения, похожие на прогресс. Потеря цели — диффы идут, но никто не задаёт вопрос о текущем этапе. Архитектурный перегиб — усложнение до подтверждения базового пути. Допущения — догадки «должно работать так»._
...
- [✓ правильный] Вынести её в backlog этапа
-  Сразу включить её в текущий многошаговый diff
-  Спрятать её внутри рефакторинга

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _По теории, всё что не нужно для завершения текущего этапа, либо получает отдельную контрольную точку позже, либо идёт в список доработок. Это защищает от расползания границ задачи._
...
**Варианты:**
- [✓ правильный] Формат передачи контекста между сессиями
- [✓ правильный] От 3 до 5 этапов с разной функцией
- [✓ правильный] Список сигналов остановки
-  Немедленную реализацию всей задачи

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В уроке прямо перечислено, что должно появиться в результате практики: цель, 3-5 этапов с разной функцией, артефакты и критерии, сигналы остановки и формат передачи контекста. Немедленная реализация задачи не входит — практика про планирование маршрута, а не исполнение._
