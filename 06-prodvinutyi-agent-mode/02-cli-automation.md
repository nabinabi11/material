...
- [✓ правильный] Стабильный smoke-прогон
-  Спорный выбор архитектуры
-  Редкий исследовательский заход

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано, что лучший кандидат для CLI-автоматизации — действие с предсказуемым входом и наблюдаемым выходом, например короткая smoke-проверка. Остальные варианты — неустоявшиеся процессы, которые сначала надо стабилизировать вручную._
...
- [✓ правильный] Результат зависит от каждой новой ситуации
- [✓ правильный] Порядок действий всё ещё меняется
-  Команда уже стабильно проходится вручную

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Теория прямо говорит: опасно автоматизировать неустоявшийся процесс — когда структура проверки меняется и последовательность зависит от ситуации. Стабильное ручное прохождение и описуемые входы/выходы — наоборот, признаки хорошего кандидата._
...
- [✓ правильный] Хук срабатывает по событию
-  CLI-запуск исключает явный checkpoint

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано: CLI-команду запускают явно, а хук срабатывает автоматически при наступлении события в lifecycle агента — это и есть принципиальное отличие._
...
- Claude Code hooks
- Codex hooks
- Cursor format-on-save
- CLI-обёртка

**Колонка Б (варианты, перемешаны):**
- типичный путь локального автоформатирования
- пока перехватывают PostToolUse только для Bash
- явная точка входа с известным результатом
- могут запускаться после Edit|Write|MultiEdit
...
**Мой reasoning:** _В уроке прямо: Claude Code matcher Edit|Write|MultiEdit; Codex PreToolUse/PostToolUse пока только для Bash; в Cursor для автоформатирования опираются на форматирование в редакторе; CLI-обёртка — единая предсказуемая точка входа с описанным результатом._
...
**Варианты:**
- [✓ правильный] PostToolUse
- [✓ правильный] PreToolUse
-  SessionEnd
- [✓ правильный] PostToolUseFailure

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо указано, что класс «Перехват действий инструментов» включает PreToolUse (срабатывает до выполнения), PostToolUse (после успешного выполнения) и PostToolUseFailure (добавляет контекст об ошибке). SessionEnd относится к жизненному циклу сессии._
...
- [✓ правильный] Они ловят только Bash
-  Им нужен отдельный MCP-сервер

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В теории прямо сказано: PreToolUse и PostToolUse в Codex срабатывают только для инструмента Bash, поэтому перехватить события редактирования файлов через apply_patch нельзя — это и блокирует сценарий post-edit форматирования._
...
**Варианты:**
- [✓ правильный] Обязательные входы команды
- [✓ правильный] Коды завершения и логирование
-  Только внутреннюю реализацию скрипта
- [✓ правильный] Выходы и артефакты

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В тексте automation contract явно описывает входы (аргументы, env, состояние дерева), выходы (stdout, файлы, артефакты) и коды завершения с логированием. Внутренняя реализация наоборот считается менее важной, чем явно описанные выходы._
...
**Варианты:**
- [✓ правильный] Чтобы маршрут различал тип исхода
...
**Мой reasoning:** _В теории прямо сказано: разделённые числовые коды позволяют агенту по-разному реагировать на успех, ошибку во входах и провал проверки — то есть различать тип исхода в маршруте._
...
**Варианты:**
- [✓ правильный] Их проще комбинировать в маршрут
- [✓ правильный] У них яснее ответственность
- [✓ правильный] Их результат легче проверить
-  Они уменьшают явность этапов

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _Узкие команды имеют одну функцию (ясная ответственность), предсказуемый выход (легче проверить) и комбинируются агентом в маршруте. Вариант про 'уменьшают явность этапов' противоречит идее checkpoint-блоков._
...
- [✓ правильный] Опора на format-on-save и правила проекта

**Статус Stepik:** `correct` (score 1.0)

**Мой reasoning:** _В уроке прямо сказано: в Cursor для автоформатирования обычно опираются не на агентный хук, а на форматирование в редакторе и правила проекта._
